...
public class StringUtil {
    ...
    public static String splitByBit(String str, int bitLength){
        ...
        try{
            ...
            for(int temp = 0; temp < bitLength;){
                ...
                byte[] by = String.valueOf(ch).getBytes("UTF-8");
                if(by.length > 1){
                    temp = temp + 2;
                }else{
                    temp++;
    ...
    */
    ...
    public static String getString(byte[] b, int offset, int len, String encoding){
        String ret = "";
        try{
            ret = new String(b, offset, len, encoding);
        }catch(UnsupportedEncodingException e){
            e.printStackTrace();
        }
        return ret;
    ...
    public static String objToString(Object o){
        return com.zj.config.common.StringUtil.replace(String.valueOf(o));
    }
    ...
    /***
     * @Title 将数字转为字符串时前面补0，例如 9 -> 0009
     * @author jacky
     * @date 2019年7月10日 上午10:19:30 
     * @param targetNumber
     * @param targetNumberStr
     * @return
     * String
     */
    public static String supplyZeroToString(Long targetNumber, String targetNumberStr){
        String result = null;
        try{
            int length = targetNumberStr.length();
            String codeFormat = "%0" + String.valueOf(length) + "d";//%03d
            result = String.format(codeFormat, targetNumber);
        }catch(Exception e){
            e.printStackTrace();
        }
        return result;
