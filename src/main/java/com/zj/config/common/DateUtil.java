...
public final class DateUtil {
    ...
    public static boolean isDate10Before(String date1, String date2){
        ...
        date1 = date1.replace("-", "").replace(" ", "").trim();
        date2 = date2.replace("-", "").replace(" ", "").trim();
        long date1_ = Long.parseLong(date1);
        long date2_ = Long.parseLong(date2);
        long diff = date1_ - date2_;
        if(diff >= 0){//date1晚于date2
            ret = false;
        }else if(diff < 0){
            ret = true;
        }
        ...
        //            return df.parse(date1.toString()).before(df.parse(date2.toString()));
        //        }catch(ParseException e){
        //            e.printStackTrace();
        //            return false;
        //        }
    ...
    public static String giveMonthLast(String strdate){
        // 先得到下个月的同一天
        String addmonth = com.zj.config.common.DateUtil.addMonth(strdate);

        // 得到下个月的月初
        String monthfirst = com.zj.config.common.DateUtil.giveMonthFist(addmonth);

        // 下个月月初减一天为本月月末
        String subday = com.zj.config.common.DateUtil.subDay(monthfirst);
        return subday;
    }
    ...
    public static String giveBeforeMonthFirst(String strdate){
        // 调用得到上个月的函数
        String beforemonth = com.zj.config.common.DateUtil.subMonth(strdate);

        // 调用构造月初的函数
        return com.zj.config.common.DateUtil.giveMonthFist(beforemonth);
    }
    ...
    public static String giveBeforeMonthLast(String strdate){
        // 先调用函数得到本月月初
        String monthfirst = com.zj.config.common.DateUtil.giveMonthFist(strdate);

        // 调用当前日期减一天方法得到上个月月末
        return com.zj.config.common.DateUtil.subDay(monthfirst);
    }
    ...
    public static String addHours(Date startDate, int intHour){
        try{
    ...
    public static String getThisMonthLast(String strdate){
        ...
        // 先得到下个月的同一天
        String addmonth = com.zj.config.common.DateUtil.addMonth(thisStrDate);
     ...
     * 当前时间增加间隔秒后的时间
     */
    public static String addSeconds(Date startDate, int seconds){
        try{
            SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
            Calendar cal = Calendar.getInstance();
            cal.setTime(startDate);
            long longMills = cal.getTimeInMillis() + seconds * 1000;
            cal.setTimeInMillis(longMills);
    ...
    */
     ...
     * @return
     */
    public static String getDateToString(Date date_, String dateformat){
        SimpleDateFormat dateFormat = new SimpleDateFormat(dateformat);// 可以方便地修改日期格式
     ...
     * @throws ParseException
     */
    public static Date getStringToDate(String date_, String dateformat){
        Date date = null;
     ...
     * @return
     */
    public static int getDays(String date_, String dateformat){
        int days = 0;
        try{
            Calendar ca = Calendar.getInstance();//创建一个日期实例
            SimpleDateFormat sdf = new SimpleDateFormat(dateformat);//格式化为年月日
            ca.setTime(sdf.parse(date_));//实例化一个日期
            days = ca.get(Calendar.DAY_OF_YEAR);//获取是第多少天
        }catch(Exception e){
            e.printStackTrace();
        }
        return days;
     ...
     * @return yyyy-MM-dd
     */
    public static String getTransDate(int year, long days, String dateformat){
        String date_res = "";
        try{
            String startDate = (year - 1) + "-12-31";
            Date date = getStringToDate(startDate, "yyyy-MM-dd");
            long dateM = date.getTime();
            days = days * 24 * 60 * 60 * 1000; // 要加上的天数转换成毫秒数
            dateM += days; // 相加得到新的毫秒数
            date_res = getDateToString(new Date(dateM), dateformat); // 将毫秒数转换成日期
        }catch(Exception e){
            e.printStackTrace();
        }
        return date_res;
    ...
    /**
     * 得到第一个日期相对于第二个日期的偏移天数，第一个日期要大于第二个日期
     * @param endDate
     * @param startDate
     * @return
     */
    public static long getMoveDays(String endDate, String startDate){
        if(StringUtil.isNullOrEmpty(endDate) || StringUtil.isNullOrEmpty(startDate)){
            return 0;
        }
    ...
    public static List<Date> findDates(Date dBegin, Date dEnd){
        ...
        if(!getDateToString(dBegin, "yyyy-MM-dd").equals(getDateToString(dEnd, "yyyy-MM-dd"))){
            lDate.add(dEnd);
        }
     ...
     * @param beginDate
     * @param endDate
     * @param putfomart 输入的日期格式
     * @param retrurnfomart 返回的日期格式
     * @return
     */
    @SuppressWarnings("unchecked")
    public static List<String> findDates(String beginDate, String endDate, String putfomart, String retrurnfomart){
        List<String> listStr = new ArrayList<String>();
        Date dBegin = getStringToDate(beginDate, putfomart);
        Date dEnd = getStringToDate(endDate, putfomart);
        if(dEnd.compareTo(dBegin) >= 0){
            List<Date> list = findDates(dBegin, dEnd);
            for(Date date : list){
                listStr.add(getDateToString(date, retrurnfomart));
            }
    ...
    /**
     * 获取指定日期的下一个月的最后一天
     * @param dateStr 类型yyyy-MM-dd 2021-06-16
     * @return 类型yyyy-MM-dd 2021-07-31
     */
    public static String getLastDayOfNextMonth(String dateStr){
        String nextMonth = getNextMonth(dateStr);
        String nextMonthLastDay = getLastDayOfMonth(nextMonth);
    ...
    public static String getLastDayOfPreMonth(){
        return giveBeforeMonthLast(getCurrentDate());
    }
    ...
    public static int getAgeByBirth(String birthday, String format) {
        int age = 0;
        try {
            Date birthDate = getStringToDate(birthday,format);
     ...
     * @return
     */
    public static long getMonthDiff(Date birthDay) {
        long mm = 0;
        try {
            String beginDStr = getDateToString(birthDay,"yyyy-MM-dd");
            String nowDStr = getNowTime("yyyy-MM-dd");
            DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd");
            LocalDate beginD = LocalDate.parse(beginDStr,formatter);
            mm = ChronoUnit.MONTHS.between(beginD,LocalDate.parse(nowDStr,formatter));
            return mm;
        } catch (Exception e) {//兼容性更强,异常后返回数据
            return 0;
        }
     ...
     * @return
     */
    public static long getMonthDiff(String beginDStr) {
        long mm = 0;
        try {
            String nowDStr = getNowTime("yyyy-MM-dd");
            beginDStr = beginDStr.split(" ")[0];
            DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd");
            LocalDate beginD = LocalDate.parse(beginDStr,formatter);
            mm = ChronoUnit.MONTHS.between(beginD,LocalDate.parse(nowDStr,formatter));
            return mm;
    ...
    public static void main(String[] args) throws ParseException{
        ...
        //        System.out.println(getDays("2019-01-01","yyyy-MM-dd"));
        //        System.out.println(getDays("2019-10-09","yyyy-MM-dd"));
        //        System.out.println(getTransDate(2019,1,"yyyy-MM-dd"));
        //        System.out.println(getMoveDays("2019-01-02","2019-01-01"));

        //        System.out.println(isDate10Before("2019-11-22","2019-11-23"));
        //        System.out.println(isDate10Before("2019-11-23","2019-11-23"));
        //        System.out.println(isDate10Before("2019-11-25","2019-11-23"));
        //        System.out.println(getLastDayOfMonth(2019,12));

        //        for(String dateStr : findDates("2017-02-10", "2017-12-02", "yyyy-MM-dd", "yyyy-MM-dd")){
        //            System.out.println(dateStr);
        //        }
        // System.out.println(getNextMonth("2021-06-16"));
        // System.out.println(getLastDayOfMonth("2021-10-08"));

        // System.out.println(giveBeforeMonthLast("2021-06-16"));
        // System.out.println(getLastDayOfPreMonth());

//        System.out.println(getAgeByBirth("2021-10-16","yyyy-MM-dd"));
//        System.out.println(getAgeByBirth("2021-06-16","yyyy-MM-dd"));
//        System.out.println(getAgeByBirth("2019-08-01","yyyy-MM-dd"));

//        System.out.println(getMonthDiff("2021-10-16"));
//        System.out.println(getMonthDiff("2021-06-16 00:00:00"));
//        System.out.println(getMonthDiff("2019-08-01 00:00:00"));
